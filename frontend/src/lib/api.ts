...
export const GetSessionDetails = async (sessionId: string) => {
    const response = await axios.get(`${HostURL}/v1/authentication/sessions/${sessionId}`, {
        headers: {
...
export const SendGovernmentID = async (userID: string) => {
    try {
        const response = await axios.post(
            ...
            {
                credentialSchema: "ipfs://QmfQhst5Kn3zDP8haepknsPfuTwRzQYdbR6cnDYK7U1Pni",
                credentialSubject: {
                    National_Id: "IND0003",
                    DOB: 19990101,
                    id: userID,
...
export const SendEmploymentVC = async (userID: string) => {
    try {
        const response = await axios.post(
            `${HostURL}/v1/credentials`,
            {
                credentialSchema: "ipfs://QmVCHRiLmGRXpL1TCzoLCwnUBUZUzrRsicwLLtkSummdJ5",
                credentialSubject: {
                    salary: 1000000,
...
export const SendLabVC = async (userID: string) => {
    try {
        const response = await axios.post(
            `${HostURL}/v1/credentials`,
            {
                credentialSchema: "ipfs://QmYawPmEhp6Qmsyxb56LJkHrgpL1HiPCqb4WhP3VXkCueg",
                credentialSubject: {
                    bmi: 2312,
                    heart_rate: 8300,
                    blood_sugar: 13672,
...
export const SendInsuranceVC = async (userID: string) => {
    try {
        const response = await axios.post(
            `${HostURL}/v1/credentials`,
            {
                credentialSchema: "ipfs://QmbDA7qeACRHWbQpY7ogoQdUkeqP2gkEJj8HmTcXfGRcVK",
                credentialSubject: {
...
export const fetchGovtQRCode = async (sessionId: string) => {
    ...
    const response = await axios.get(`http://localhost:4007/verify/govt-auth-qr?sessionId=${sessionId}`, {
        headers: {
            authorization: import.meta.env.VITE_AUTH,
        },
    })
...
export const fetchInsuranceQR = async (sessionId: string) => {
    console.log("called multiple times")
    const response = await axios.get(`http://localhost:4007/verify/insurance-issue-auth-qr?sessionId=${sessionId}`, {
        headers: {
            authorization: import.meta.env.VITE_AUTH,
        },
    })
